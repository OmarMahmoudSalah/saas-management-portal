...
export function SaaSAdminDashboard() {
  ...
  return (
    <div style={{ padding: '24px 32px', background: 'var(--bg-main, #0f172a)', color: 'var(--text-dark, #f8fafc)', minHeight: '100vh', fontFamily: 'var(--font-main, sans-serif)' }}>
      {/* Header Bar */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ background: 'linear-gradient(135deg, #6366f1, #a855f7)', width: 44, height: 44, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ffffff', fontWeight: 800, fontSize: 22, boxShadow: '0 4px 14px rgba(99,102,241,0.4)' }}>
            D
          </div>
          <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button 
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: 'var(--bg-card, #1e293b)', border: '1px solid var(--border, #334155)', color: 'var(--text-dark, #ffffff)', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 28 }}>
        <div style={{ background: 'var(--bg-card, #1e293b)', border: '1px solid var(--border, #334155)', borderRadius: 14, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', fontWeight: 700, textTransform: 'uppercase' }}>Monthly Recurring Revenue</span>
            <DollarSign size={18} color="#10b981" />
          </div>
          <div style={{ fontSize: 28, fontWeight: 800, color: '#10b981' }}>${totalMrr} / mo</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', marginTop: 4 }}>Live calculated subscription MRR</div>
        </div>

        <div style={{ background: 'var(--bg-card, #1e293b)', border: '1px solid var(--border, #334155)', borderRadius: 14, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', fontWeight: 700, textTransform: 'uppercase' }}>Annual Run Rate</span>
            <Activity size={18} color="#6366f1" />
          </div>
          <div style={{ fontSize: 28, fontWeight: 800, color: '#6366f1' }}>${totalArr} / yr</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', marginTop: 4 }}>Contracted annual revenue run rate</div>
        </div>

        <div style={{ background: 'var(--bg-card, #1e293b)', border: '1px solid var(--border, #334155)', borderRadius: 14, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', fontWeight: 700, textTransform: 'uppercase' }}>Active App Instances</span>
        ...
        <div style={{ background: 'var(--bg-card, #1e293b)', border: '1px solid var(--border, #334155)', borderRadius: 14, padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', fontWeight: 700, textTransform: 'uppercase' }}>Hardware Anti-Cloning</span>
            <Shield size={18} color="#f59e0b" />
          </div>
          <div style={{ fontSize: 20, fontWeight: 800, color: '#34d399', marginTop: 6 }}>ACTIVE ENFORCED</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted, #94a3b8)', marginTop: 4 }}>Machine ID fingerprinting enabled</div>
        </div>
      </div>

      {/* Tabs Navigation */}
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, borderBottom: '1px solid var(--border, #334155)', paddingBottom: 12 }}>
        {[
      ...
      {activeTab === 'clients' && (
        <div style={{ background: 'var(--bg-card, #1e293b)', borderRadius: 14, border: '1px solid var(--border, #334155)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border, #334155)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <Building2 size={18} color="var(--primary, #6366f1)" />
              <span style={{ fontWeight: 700, fontSize: 15 }}>Real-Time Client Instances ({filteredClients.length})</span>
            </div>

            <div style={{ position: 'relative', width: 280 }}>
              <Search size={15} style={{ position: 'absolute', left: 10, top: 10, color: 'var(--text-muted, #94a3b8)' }} />
              <input
                type="text"
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                placeholder="Search organization or HW ID..."
                style={{ width: '100%', padding: '8px 12px 8px 32px', background: 'var(--bg-main, #0f172a)', border: '1px solid var(--border, #334155)', borderRadius: 8, color: '#ffffff', fontSize: 12 }}
              />
            </div>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid var(--border, #334155)', color: 'var(--text-muted, #94a3b8)', textTransform: 'uppercase', fontSize: 11 }}>
            ...
            <tbody>
              {filteredClients.map(c => (
                <tr key={c.id} style={{ borderBottom: '1px solid var(--border, #334155)' }}>
                  <td style={{ padding: '14px 16px' }}>
                    ...
                    <div style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--text-muted, #94a3b8)' }}>{c.id}</div>
                  </td>
                  <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                    <span style={{ background: c.plan === 'ENTERPRISE' ? 'rgba(168,85,247,0.2)' : c.plan === 'GROWTH' ? 'rgba(99,102,241,0.2)' : 'rgba(148,163,184,0.2)', color: c.plan === 'ENTERPRISE' ? '#c084fc' : c.plan === 'GROWTH' ? '#818cf8' : '#cbd5e1', padding: '4px 10px', borderRadius: 8, fontSize: 11, fontWeight: 800 }}>
                      {c.plan || 'GROWTH'}
                    </span>
                  </td>
                  <td style={{ padding: '14px 16px', textAlign: 'center' }}>
              ...
              {filteredClients.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted, #94a3b8)' }}>
      ...
      {activeTab === 'license' && (
        <div style={{ background: 'var(--bg-card, #1e293b)', borderRadius: 14, border: '1px solid var(--border, #334155)', padding: 24, maxWidth: 640 }}>
          <h3 style={{ margin: '0 0 8px 0', fontSize: 16, fontWeight: 800, color: '#ffffff', display: 'flex', alignItems: 'center', gap: 10 }}>
            <Key size={20} color="var(--primary, #6366f1)" />
            <span>Hardware Anti-Cloning License Token Generator</span>
          </h3>
          <p style={{ fontSize: 13, color: 'var(--text-muted, #94a3b8)', marginBottom: 20 }}>
          ...
          <form onSubmit={handleGenerateLicense}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted, #94a3b8)', marginBottom: 6 }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '10px 14px', background: 'var(--bg-main, #0f172a)', border: '1px solid var(--border, #334155)', borderRadius: 8, color: '#ffffff', fontSize: 13, fontFamily: 'monospace' }}
              />
            </div>

            <button type="submit" style={{ width: '100%', padding: '10px 16px', background: 'var(--primary, #6366f1)', color: '#ffffff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              🔑 Generate Hardware License Token
            </button>
          </form>

          {generatedToken && (
            <div style={{ marginTop: 20, padding: 16, background: 'var(--bg-main, #0f172a)', border: '1px dashed var(--primary, #6366f1)', borderRadius: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#818cf8', textTransform: 'uppercase' }}>Signed License Token:</span>
